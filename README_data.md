...
当前 Codex PowerShell 环境中 `python` 不在 PATH 里，运行脚本时可以使用本机 Python 绝对路径：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --dry-run
```

下载脚本默认只处理三个类别的 `train`、`valid`、`test`，不会下载 `review` 和 `meta`。建议先用 dry-run 确认计划：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --dry-run

& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --categories Industrial_and_Scientific --dry-run
```

单类别真实下载测试：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --categories Industrial_and_Scientific
```

全部类别必需 split 下载：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --categories all
```

如确实需要可选的大体积原始 review/meta 文件，再显式启用：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --categories all --include-review-meta
```

URL 轻量检查不会下载完整文件：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/download_amazon5core.py --categories all --check-url
```

预处理和检查脚本在完成下载后再运行：

```powershell
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/preprocess_to_sasrec.py --help
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/check_raw_data.py --help
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/check_processed_data.py --help
& "C:\Users\Chess\AppData\Local\Programs\Python\Python312\python.exe" scripts/make_data_readme.py --help
```

注意：`data/raw/` 和 `data/processed/` 下的数据文件不应提交到 Git。
