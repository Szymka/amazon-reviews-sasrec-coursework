...
class SeqDataset_Inference(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        neg = np.array(neg)
        return user_id, seq, pos, neg
def _default_amazon_interaction_path(fname):
    """仓库内 `data/amazon/<fname>.txt`，相对本文件定位，避免依赖当前工作目录。"""
    here = os.path.dirname(os.path.abspath(__file__))
    return os.path.normpath(os.path.join(here, "..", "..", "data", "amazon", "%s.txt" % fname))
...
def data_partition(fname, path=None):
    ...
    data_path = path if path is not None else _default_amazon_interaction_path(fname)
    f = open(data_path, "r")
    for line in f:
